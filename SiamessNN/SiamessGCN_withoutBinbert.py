...
from dataprocess_withoutBinbert import *
import torch
import torch.nn as nn
import torch.nn.functional as F
from torch_geometric.nn import GCNConv, global_mean_pool
...
class GraphEncoder(nn.Module):
    def __init__(self, input_dim, hidden_dim1, hidden_dim2, output_dim):
        ...
        # 定义多个GCN层
        self.conv1 = GCNConv(input_dim, hidden_dim1)  # (11, 128)
        self.conv2 = GCNConv(hidden_dim1, hidden_dim2)  # (128, 256)
        self.conv3 = GCNConv(hidden_dim2, hidden_dim2)  # (256, 256)
        self.dropout = nn.Dropout(0.3)  # Dropout
        self.fc = nn.Linear(hidden_dim2, output_dim)  # (256, 128) 
...
# 三元组训练函数
def train(model, loader, optimizer, loss_fn, device, epoch, save_dir="./model_withoutBinbert/TripletGNN", 
          checkpoint_path="./model_withoutBinbert/latest_checkpoint.pth", log_file="./training_withoutBinbert.log"):
...
if __name__ == "__main__":
    ...
    save_path = "./model_withoutBinbert/SiameseGNN_withoutBinbert"  # 模型保存路径
    save_interval = 1  # 每 1 轮保存一次模型
    checkpoint_path="./model_withoutBinbert/latest_checkpoint.pth"
    log_file="./training_withoutBinbert.log"
    ...
    train_loader, val_loader = prepare_triplet_datasets(csv_file, test_size=0.2, batch_size=batch_size)
