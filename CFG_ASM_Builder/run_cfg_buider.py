...
LOG_PATH = ".\log\cfg_builder_log.txt"
IDA_PLUGIN = join(dirname(abspath(__file__)), 'IDA_cfg_builder.py')
...
@click.option('-f', '--fname', default=None, help='Optional: Specify a function name to generate CFG and disassembly only for this function.')
def main(target_file, output_dir, ida_path, fname):
    try:
        ...
        cmd = [
            ...
            '-Ocfg_builder:{};{};{}'.format(target_file, output_dir, fname if fname else ""),  # 插件选项
            target_file
        ]
